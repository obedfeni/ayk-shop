...
export function LoginForm({ onCancel, onSuccess }: LoginFormProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (ok) {
      onSuccess();
      // Force router refresh so admin page re-renders immediately
      router.refresh();
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: COLORS.bg }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        className="w-full max-w-md bg-white rounded-3xl p-8 shadow-2xl border-2"
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="relative">
            ...
            <button type="button" onClick={() => setShowPwd(!showPwd)}
              className="absolute right-3 bottom-2.5 text-ayk-text-muted hover:text-ayk-text">
